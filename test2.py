from strategy_teacher_oracle import *
    ...
    #import dynet as dy
    #import numpy as np
def run1(use_oracle_training=False,use_oracle_testing=False):
    ...
    for tests in range(2):
            ...
            for (lookahead, epsilon) in [(3, 0)]:
                ...
                for training in range(number_of_epochs):
                    ...
                    for length in range(1, training_execution_length):
                        ...
                        pve.generate_training_execution(length, lookahead=lookahead,
                                                        epsilon=epsilon,use_oracle=use_oracle_training)  # compare_loss = False)
        ...
        for control in range(C):
            pve.reinitialize()
            execution = pve.generate_controlled_execution(L_C_short,use_oracle=use_oracle_testing)  # ,print_probs = False)
            failures_short.append(count_failures(execution))

        for control in range(C):
            pve.reinitialize()
            execution = pve.generate_controlled_execution(L_C,use_oracle=use_oracle_testing)  # ,print_probs = False)
