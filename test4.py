...
def createByNum(num_model):
    ...
    if (num_model == 1):
        print("CASES")
        pve = create_fig1_1_cases(model)  # this generates the system and environment
    if (num_model == 2):
        print("STRATEGY")
        pve = create_fig1_2_strategy_oracle(model)  # this generates the system and environment
    if (num_model == 3):
        pve = create_fig1_3_combination_lock_oracle(model)  # this generates the system and environment
...
def run1(use_oracle_training=False, use_oracle_testing=False,num_model=0):
    ...
    def train(pve, use_oracle_training):
        number_of_epochs = 50  # this is what we called T so far
        training_execution_length = 50  # this is what we called L so far
        runs = 1  # will be >1 only for combined examples
        lookahead = 0
    ...
    for tests in range(2):
        ...
        pve=train(pve, use_oracle_training)
